...
- Roam targets must actually be farmable: species with spawn-once or very-long respawns (`ROAM_MAX_RESPAWN`, default 10 min) are ignored, so the bot stops routing to Spooky Town's Jr. whose 7.2h respawn means the spot is almost always empty.
...
- Full-bank handling: the tidy loop now respects `skipCache`, so a spuriously failing action can no longer reset the trip counter and strand the bot in town forever. When the bank hits `BANK_MAX_ITEMS`, junk-bin drops are sold instead of banked (`BANK_FULL_SELL`) so inventory clears and the bot returns to fighting; protected `BANK_NAMES` items are never sold and just stay carried.
- Bank exits before long paths: after depositing, the character never cross-map routes from inside the `bank` building — it walks out to the mainland town square (`BANK_EXIT`) first, then resumes normal targeting/roaming (both `roam()` and `go_back()` gate on `leave_bank()`).
